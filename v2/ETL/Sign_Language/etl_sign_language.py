...
dict_hand_gesture = {0 : 'a', 1 : 'a', 2 : 'l', 3 : 'l', 4 : 'y', 5: 'y' , 6: 'u', 7 : 'u', 8: '5-hand', 9 : '5-hand'}
dict_palm_orientation = {0 : 'front', 1 : 'back'}


model_hand_gesture_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"model_hand_gesture.p")
model_palm_orientation_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"model_palm_orientation.p")
...
def predict_hand_gesture(landmarks):
    model_dict = pickle.load(open(model_hand_gesture_path, 'rb'))
    model = model_dict['model']
    predicted_index = int((model.predict(landmarks))[0])
    decision = dict_hand_gesture[predicted_index]
    return decision

def predict_palm_orientation(landmarks):
    model_dict = pickle.load(open(model_palm_orientation_path, 'rb'))
    model = model_dict['model']
    predicted_index = int((model.predict(landmarks))[0])
    decision = dict_palm_orientation[predicted_index]
    return decision
...
def load_decision(cnx, hand_gesture:str, palm_orientation: str):
    cursor = cnx.cursor()
    cursor.execute("SELECT `decision_id` FROM `Decision` WHERE `hand_gesture` = '{}' AND `palm_orientation` = '{}'".format(hand_gesture, palm_orientation))
    result = cursor.fetchone()
    if result is None:
        sql = "INSERT INTO `Decision` (`hand_gesture`, `palm_orientation`) VALUES (%s, %s)"
        val = (hand_gesture, palm_orientation)
        cursor.execute(sql, val)
        cnx.commit()
        cursor.close()
        return cursor.lastrowid
...
def load_fact(cnx,video_id : int, frame_id : int, decision_id : int):
    cursor = cnx.cursor()
    sql = "INSERT INTO Fact (video_id, frame_id, decision_id) VALUES (%s, %s, %s)"
    val = (video_id, frame_id, decision_id)
    cursor.execute(sql, val)
    cnx.commit()
    cursor.close()
    return cursor.lastrowid
...
def process_video(video_path):
    ...
    while True:
        ...
        if frame_duration >= closest_duration:
            ...
            palm_orientation = "_NO DECISION_"
            hand_gesture = "_NO DECISION_"

            if has_landmarks:
                hand_gesture = predict_hand_gesture(hand_landmarks)
                palm_orientation = predict_palm_orientation(hand_landmarks)

            try:
                # Load frame
                frame_id = load_frame(cnx, has_landmarks, path, frame_nbr, encode_landmarks(hand_landmarks) if has_landmarks else "", (palm_orientation != "_NO DECISION_" and hand_gesture != "_NO DECISION_"))

                # Load decision
                decision_id = load_decision(cnx, hand_gesture = hand_gesture, palm_orientation = palm_orientation)

                # Load fact
                load_fact(cnx, video_id, frame_id, decision_id)
